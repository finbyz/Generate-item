...
def get_data(filters):
	...
	query = (
		...
		.select(
			...
			po_item.description,
			po_item.qty,
			po_item.stock_qty.as_("stock_qty"), 
			...
			po_item.custom_batch_no.as_("custom_batch_no"),  # ✅ NEW
			po_item.custom_drawing_no,
			po_item.custom_drawing_rev_no,
			po_item.custom_purchase_specification_no,
			po_item.custom_purchase_specification_rev_no,
			po_item.custom_pattern_drawing_no,
			po_item.custom_pattern_drawing_rev_no,
	...
	# Batch No Filter (MultiSelect)
	batch_no = filters.get("batch_no") or []

	if isinstance(batch_no, str):
		batch_no = [b.strip() for b in batch_no.split(",") if b.strip()]

	if batch_no:
		query = query.where(po_item.custom_batch_no.isin(batch_no))
