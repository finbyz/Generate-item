...
def get_data(filters):
	...
	query = (
		...
		.on(mr_item.parent == mr.name)
		  # PO Join
		.left_join(po_item).on(
			(po_item.material_request == mr.name)
			& (po_item.material_request_item == mr_item.name)
		)
		.left_join(po).on(po.name == po_item.parent)

		# Purchase Receipt Join
		.left_join(pr_item).on(
			(pr_item.purchase_order == po.name)
			& (pr_item.item_code == mr_item.item_code)
		)
		.left_join(pr).on(pr.name == pr_item.parent)
		.select(
			mr.name.as_("material_request"),
			mr.transaction_date.as_("date"),
			mr_item.schedule_date.as_("required_date"),
			mr_item.custom_batch_no.as_("custom_batch_no"), 
			mr_item.branch.as_("branch"),
			...
			# Request Qty
			mr_item.qty.as_("request_qty"),
			mr_item.uom.as_("request_uom"),

			# PO Data
			po.name.as_("vendor_po_no"),
			po.transaction_date.as_("vendor_po_date"),
			po.schedule_date.as_("po_required_by"),
			po.supplier.as_("vendor_name"),

			po_item.stock_qty.as_("po_qty_stock"),
			po_item.stock_uom.as_("po_stock_uom"),
			po_item.qty.as_("po_qty_uom"),
			...
			pr_item.received_stock_qty.as_("received_qty_stock"),
			pr_item.stock_uom.as_("received_stock_uom"),
			(Coalesce(po_item.stock_qty, 0) - Coalesce(pr_item.stock_qty, 0)).as_("balance_qty"),
			pr_item.received_qty.as_("received_qty_uom"),
