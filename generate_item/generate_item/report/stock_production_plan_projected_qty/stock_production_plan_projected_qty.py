...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		# Get Production Plan Request Qty for this item and warehouse
		production_plan_request_qty = get_production_plan_request_qty(bin.item_code, bin.warehouse)
...
def get_columns():
	return [
		...
		{
			"label": _("Production Plan Request Qty"),
			"fieldname": "production_plan_request_qty",
			"fieldtype": "Float",
			"width": 120,
			"convertible": "qty",
		},
...
def get_production_plan_request_qty(item_code, warehouse):
	"""Get the total requested quantity for an item from Production Plan Material Request Items (mr_items) table"""
	query = """
		SELECT COALESCE(SUM(mri.requested_qty), 0) as total_qty
		FROM `tabProduction Plan` pp
		INNER JOIN `tabMaterial Request Plan Item` mri ON pp.name = mri.parent
		WHERE pp.docstatus = 1
		AND pp.status = 'Completed'
		AND mri.item_code = %s
		AND mri.warehouse = %s
		AND mri.parentfield = 'mr_items'
	"""
	
	result = frappe.db.sql(query, (item_code, warehouse))
