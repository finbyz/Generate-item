...
class TestValveAssembly(unittest.TestCase):
	"""
	Integration tests for ValveAssembly naming series deletion behavior.
	"""

	def setUp(self):
		frappe.set_user("Administrator")
		frappe.publish_realtime = lambda *args, **kwargs: None
		self.test_branch = "Sanand"  # series: VASS.fiscal.#####
		self.naming_series = "VASS.fiscal.#####"
		self.created_docs = []
	...
	def cleanup_records(self):
		for name in getattr(self, "created_docs", []):
			if frappe.db.exists("Valve Assembly", name):
				docstatus = frappe.db.get_value("Valve Assembly", name, "docstatus")
				if docstatus == 1:
					try:
						doc = frappe.get_doc("Valve Assembly", name)
						doc.cancel()
					except Exception:
						pass
				try:
					frappe.delete_doc("Valve Assembly", name, force=1, delete_permanently=True)
				except Exception:
					pass
		self.created_docs = []

	def _create_doc(self, submit=False):
		doc = frappe.new_doc("Valve Assembly")
	...
	def test_1_delete_latest_reuses_number(self):
		...
		d3_name = d3.name
		frappe.delete_doc("Valve Assembly", d3_name)
	...
	def test_2_delete_non_latest_does_not_reduce_counter(self):
		...
		frappe.delete_doc("Valve Assembly", d2.name)
	...
	def test_4_repeated_delete_and_recreate(self):
		...
		# Delete 003 again
		frappe.delete_doc("Valve Assembly", d4.name)
...
IntegrationTestValveAssembly = TestValveAssembly
