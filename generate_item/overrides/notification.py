...
class CustomNotification(Notification):
    ...
    # Debug helper
    # -------------------------------------------------------------------------
    def _debug(self, step, **data):
        frappe.log_error(
            message=f"{step}\n" + "\n".join(f"{k}: {v}" for k, v in data.items()),
            title=f"CustomNotification :: {step}",
        )
    ...
    # -------------------------------------------------------------------------
    def validate(self):
        self._debug("validate:enter")
        super().validate()
        self._validate_value_editor_requirements()
        self._debug("validate:exit")

    def get_list_of_recipients(self, doc, context):
        self._debug(
            "get_list_of_recipients:enter",
            doc=doc.name,
            doctype=doc.doctype,
            context=context,
        )
        ...
        if self._should_send_to_value_editors():
            editor_emails = self._get_value_editor_emails(doc)
            self._debug("value_editors_found", editor_emails=editor_emails)
        ...
        self._debug("get_list_of_recipients:exit", result=result)
        return result
    ...
    def _validate_value_editor_requirements(self):
        self._debug(
            "_validate_value_editor_requirements",
            send_all_value_editor=self.get("send_all_value_editor"),
            event=self.event,
        ...
        if self.event != "Value Change":
            frappe.throw(_("Value Editor requires event 'Value Change'"))

        if (self.value_changed or "").strip().lower() != "workflow_state":
            frappe.throw(_("Value Changed must be 'workflow_state'"))
    ...
    def _should_send_to_value_editors(self):
        ...
        self._debug("_should_send_to_value_editors", result=result)
        return result

    def _get_value_editor_emails(self, doc):
        self._debug("_get_value_editor_emails:enter", doc=doc.name)

        role = self._get_allow_edit_role_for_state(doc)
        self._debug("allow_edit_role", role=role)
        ...
        self._debug("value_editor_users", users=users)

        emails = self._get_primary_emails(users)
        self._debug("value_editor_emails", emails=emails)

        return emails

    def _get_allow_edit_role_for_state(self, doc):
        workflow_state = doc.get("workflow_state")
        self._debug("_get_allow_edit_role_for_state", workflow_state=workflow_state)
        ...
        for wf in workflows:
            role = frappe.db.get_value(
                "Workflow Document State",
                {"parent": wf.name, "state": workflow_state},
                "allow_edit",
            )
            if role:
                self._debug("allow_edit_role_found", workflow=wf.name, role=role)
                return role
    ...
    def _get_permitted_users_for_role_and_branch(self, role, branch, doctype):
        self._debug(
            "_get_permitted_users_for_role_and_branch",
            role=role,
            branch=branch,
            doctype=doctype,
        )
        ...
        users = frappe.db.sql(
            ...
            FROM `tabUser` u
            INNER JOIN `tabHas Role` hr
                ON hr.parent = u.name AND hr.role = %(role)s
        ...
        self._debug("permitted_users", users=users)
        return users or []

    def _get_active_users_with_role(self, role):
        self._debug("_get_active_users_with_role", role=role)
    ...
    # -------------------------------------------------------------------------
    def _filter_by_permission(self, doc, emails):
        self._debug("_filter_by_permission:enter", emails=emails)

        filtered = []
        for email in emails or []:
            user = frappe.db.get_value("User", {"email": email}, "name") or email
            allowed = self._check_user_permission(doc, user)
            self._debug("permission_check", email=email, user=user, allowed=allowed)

            if allowed:
                filtered.append(email)
            else:
                self._log_permission_denial(doc, user, email, doc.get("branch"))

        self._debug("_filter_by_permission:exit", filtered=filtered)
        return filtered

    def _check_user_permission(self, doc, user):
        self._debug("_check_user_permission", user=user)
        ...
        for p in perms:
            if p.for_value == branch and (
                p.apply_to_all_doctypes or not p.applicable_for or p.applicable_for == doc.doctype
            ):
                return True
    ...
    # -------------------------------------------------------------------------
    def _get_primary_emails(self, users):
        self._debug("_get_primary_emails", users=users)
        ...
        emails = [r.email or r.name for r in records if r.email or r.name]
        self._debug("primary_emails", emails=emails)
        return emails

    def _sanitize_email_list(self, emails):
        self._debug("_sanitize_email_list", raw=emails)

        sanitized = [
            validate_email_address(e, throw=False)
            for e in emails or []
            if validate_email_address(e, throw=False)
        ]

        self._debug("sanitized_emails", sanitized=sanitized)
        return sanitized
