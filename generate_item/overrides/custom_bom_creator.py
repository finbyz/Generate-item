...
# import frappe
# from erpnext.manufacturing.doctype.bom_creator.bom_creator import BOMCreator as CoreBOMCreator
# from generate_item.utils.bom_naming import set_bom_naming_series, get_custom_bom_name
...
#                 # Find the parent row datafrappe1223
...
#             is_duplicate = isinstance(e, (DuplicateEntryError, UniqueValidationError)) or "Duplicate" in str(e)

#             if is_duplicate:
#                 try:
#                     base = get_custom_bom_name(bom.item, getattr(bom, 'branch_abbr', None))
#                     # Find next available 3-digit suffix
#                     last = frappe.db.sql(
#                         """
...
#                     if last:
#                         try:
#                             num_part = last[0].name.split('-')[-1]
#                             next_num = int(num_part) + 1
#                         except Exception:
#                             next_num = 1
#                     manual_name = f"{base}-{next_num:03d}"
#                     bom.name = manual_name
#                     # Avoid series interference on manual name
#                     try:
#                         bom.naming_series = None
#                     except Exception:
#                         pass
#                     bom.save(ignore_permissions=True)
#                     bom.submit()
#                     production_item_wise_rm[group_key].bom_no = bom.name
#                     frappe.log_error(
#                         "BOM Created With Suffix",
#                         f"Created BOM {bom.name} for item {row.item_code} after duplicate name",
#                     )
#                 except Exception:
#                     frappe.log_error(
#                         "BOM Save/Submit Failed (Duplicate Handling)",
        ...
        try:
            ...
            if is_duplicate:
                try:
                    ...
                    frappe.log_error(
                        "BOM Created With Suffix",
                        f"Created BOM {bom.name} for item {correct_item_code} after duplicate name",
                    )
                except Exception:
                    frappe.log_error(
                        "BOM Save/Submit Failed (Duplicate Handling)",
                        f"Item: {correct_item_code}\nError: {frappe.get_traceback(with_context=True)}",
                    )
                    raise
            else:
                frappe.log_error(
