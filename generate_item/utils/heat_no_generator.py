...
@frappe.whitelist()
def generate_heat_numbers(docname):
    doc = frappe.get_doc("Quality Inspection", docname)

    if not doc.generate_heat_no:
        frappe.throw(_("Heat number generation is not enabled for this inspection."))

    series_value = doc.series_value
    start_value  = cint(doc.start_value)
    end_value    = cint(doc.end_value)
    ...
    if end_value < start_value:
     ...
     # ── UOM check: pick correct qty fields ───────────────────────────────
    ...
    full_range  = (end_value - start_value) + 1   # what user asked for
    total_qty_i = int(total_qty)

    # ── Key logic: cap rows to total_qty if range > qty ──────────────────
    ...
    # Clear existing rows before regenerating
    doc.set("heat_no", [])
